...
public class Enemy : MonoBehaviour
{
  ...
  public bool spawnPips = false;
    ...
    void UpdateLifeDisplay() {
      int start = UI.transform.childCount;
      float amt = UI.transform.childCount-life;
      for(int i=0;i<amt; i++) {
        Destroy(UI.transform.GetChild(Mathf.FloorToInt(start-i-1)).gameObject);
      }
    }
